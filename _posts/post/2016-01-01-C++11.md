...
Type safe enum is strongly typed. The enumerators are in the scope of enum and not exported to higher scope.The underlying type is also well defined for modern type safe enums. The default underlying type is int. Earlier the underlying type was int and so compiler dependent. This had the disadvantage of unknown size in packed structures.
```
...
```
Existing enums are extended to allow underlying type and explicit scoping.The names of enumerators are in the enum’s scope, and they are also injected into the enclosing scope. It preserves backward compatibility so that the meaning of existing programs is unchanged. it also enables programmers to writeenum-agnostic code on both type of enums.
